...
public class ChangeCupService {
    ...
    public void changeCup(HttpServletRequest request, HttpServletResponse respons) {
        ...
        queryMap.put("startTime", "2021-4-15 00:00:00");
        queryMap.put("endTime", "2021-4-22 23:59:59");
        ...
        for (Map tmp:selectTuisongCopData) {
            List<Map> getChangeValue = getTopValues(StringUtils.checkNull(tmp.get("eqId")),StringUtils.checkNull(tmp.get("time")));
            if(getChangeValue.size()>0){
                ...
                if(StringUtils.checkInt(getTopMap.get("dotSum"))-StringUtils.checkInt(tmp.get("warnData"))>50){
                    ...
                    tmpMap.put("time",StringUtils.checkNull(tmp.get("time")));
    ...
    @SneakyThrows
    private List<Map> getTopValues(String eqId, String time) {
